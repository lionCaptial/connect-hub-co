...
const inputClass = "mt-1 w-full rounded-xl border border-[color:var(--ch-hairline)] bg-white px-4 py-2.5 text-sm outline-none focus:border-[var(--ch-accent)] disabled:bg-[var(--ch-bg)]";
const sectionClass = "grid gap-4 rounded-2xl border border-[color:var(--ch-hairline)] p-5";
const labelClass = "text-xs font-semibold uppercase tracking-wider text-[var(--ch-ink-muted)]";
const checkboxClass = "flex items-start gap-3 text-sm text-[var(--ch-ink-muted)]";
...
function ServiceChecklist({ name }: { name: string }) {
  return <div className="grid gap-3 rounded-xl border border-[color:var(--ch-hairline)] bg-[var(--ch-bg)] p-4 sm:grid-cols-2">{serviceOptions.map((service) => <label className="flex items-center gap-2 text-sm text-[var(--ch-ink-muted)]" key={service}><input name={name} type="checkbox" value={service} />{service}</label>)}</div>;
}
...
export function ReligiousPartnerApplicationForm() {
  ...
  if (referenceId) return <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-8 text-center" role="status"><h2 className="font-serif text-2xl font-bold text-[var(--ch-ink)]">Priest Application submitted</h2><p className="mt-3 text-sm text-[var(--ch-ink-muted)]">Keep this Priest Registration Reference ID for tracking and future communication.</p><strong className="mt-4 block font-mono text-2xl text-emerald-800">{referenceId}</strong><Link className="mt-5 inline-block rounded-full bg-[var(--ch-accent-soft)] px-5 py-2.5 font-semibold text-[var(--ch-ink)]" href="/tracking?type=partner">Track Registration ID</Link></div>;

  return (
    <>
      <section className="mb-6 flex items-center gap-5 rounded-xl border border-[color:var(--ch-hairline)] bg-[var(--ch-accent)] p-5" aria-label="Priest Application information">
        <div className="min-w-0 flex-1">
          <h2 className="text-[14px] font-bold uppercase tracking-widest text-[var(--ch-accent-soft)]">Priest Registration Application</h2>
          ...
          <p className="mt-3 text-sm text-white">
            <span className="font-semibold text-[var(--ch-accent-soft)]">PLEASE MAKE SURE -</span> No immediate payment required. Fields marked * are mandatory.
          </p>
        ...
        <Link href="/contact?topic=inquiry" className="flex w-[96px] shrink-0 flex-col items-center justify-center rounded-xl border border-[color:var(--ch-hairline)] bg-[var(--ch-accent-soft)] px-2 py-2.5 text-center shadow-inner" aria-label="Raise Inquiry">
          <span className="flex h-[54px] w-[54px] items-center justify-center rounded-lg border border-[color:var(--ch-hairline)] bg-[var(--ch-accent)] [&>svg]:h-[35px] [&>svg]:w-[35px]">
            <QuestionCircleIcon />
          </span>
          <span className="mt-1.5 text-[11px] font-semibold leading-4 text-[var(--ch-ink)]">Raise Inquiry</span>
        </Link>
      </section>

      <form className="grid gap-6 rounded-2xl border border-[color:var(--ch-hairline)] bg-[var(--ch-bg-elevated)] p-6 shadow-[var(--ch-shadow-soft)] sm:p-8" onSubmit={submit}>

      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">1. Applicant Details</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">2. Address Details</h2>
        <h3 className="font-semibold text-[var(--ch-ink)]">A. Permanent Address *</h3>
        ...
        <div className="mt-2 border-t border-[color:var(--ch-hairline)] pt-5"><h3 className="font-semibold text-[var(--ch-ink)]">B. Present Service Base Address *</h3></div>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">3. Religious &amp; Professional Identity</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">4. Education cum Experience</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">5. Service Interested In</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">6. Service Interest</h2><p className="text-sm text-[var(--ch-ink-muted)]">Services the applicant is interested in accepting through Holy Yatra</p>
        <fieldset><legend className={labelClass}>Services / Rituals Interested in Accepting through Holy Yatra *</legend><ServiceChecklist name="serviceInterest" /></fieldset>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">7. Service Location &amp; Mobility</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">8. Identity &amp; Verification</h2>
        ...
        <p className="text-xs text-[var(--ch-ink-muted)]">Document fields are provided for application preparation. File-storage integration is not currently enabled.</p>
      </section>

      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">9. Operational Suitability</h2>
      ...
      <section className={sectionClass}>
        <h2 className="font-serif text-xl font-bold text-[var(--ch-ink)]">10. Declaration &amp; Consent</h2>
        ...
        <label className={checkboxClass}><input className="mt-1" name="policyConsent" type="checkbox" required />I agree to the <Link className="font-semibold text-[var(--ch-accent)] underline" href="/privacy-policy">Privacy Policy</Link> and <Link className="font-semibold text-[var(--ch-accent)] underline" href="/booking-terms">Booking Terms &amp; Conditions</Link>. *</label>
      </section>
      ...
      <label className={`${checkboxClass} rounded-xl border border-[color:var(--ch-hairline)] p-4`}><input className="mt-1" name="applicationAcknowledgement" type="checkbox" required />I understand that submission is only an application and does not guarantee approval, activation, public listing, customer assignment, or service allocation. *</label>
      <button className="rounded-full bg-[var(--ch-accent-soft)] px-6 py-3 font-semibold text-[var(--ch-ink)] transition hover:brightness-[0.98] disabled:opacity-50" disabled={submissionPending} type="submit">{submissionPending ? "Submitting Application..." : "Submit Priest Application"}</button>
      </form>
    </>
  );
}
