...
const field = "mt-1 w-full rounded-xl border border-[color:var(--ch-hairline)] bg-white px-4 py-2.5 text-sm text-[var(--ch-ink)] placeholder:text-[var(--ch-ink-muted)] outline-none focus:border-[var(--ch-accent)] [&>option]:bg-white [&>option]:text-[var(--ch-ink)]";
...
export function PublicBookingForm() {
  ...
  return <form className="grid gap-4" onSubmit={submit}>
    ...
    <label className="flex items-start gap-2"><input className="mt-1" type="checkbox" name="consent" checked={form.consent} onChange={change} required /><span className="text-sm text-[var(--ch-ink-muted)]">I consent to Holy Yatra reviewing this request and contacting me under the Privacy Policy and Booking Terms.</span></label>
    <button className="rounded-full bg-[var(--ch-accent-soft)] px-6 py-3 font-semibold text-[var(--ch-ink)] disabled:opacity-50" disabled={submitting}>{submitting ? "Submitting Request..." : "Submit Service Booking Request"}</button>
  </form>;
}
