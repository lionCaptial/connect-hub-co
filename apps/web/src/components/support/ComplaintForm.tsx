...
const field = 'mt-1 w-full rounded-xl border border-[color:var(--ch-hairline)] bg-white px-4 py-2.5 text-sm outline-none focus:border-[var(--ch-accent)] disabled:bg-[var(--ch-bg)]';
const section = 'grid gap-4 rounded-2xl border border-[color:var(--ch-hairline)] p-5';
const label = 'text-xs font-semibold uppercase tracking-wider text-[var(--ch-ink-muted)]';
...
export function ComplaintForm() {
  ...
  return <form className="grid gap-6" onSubmit={submit}>
    ...
    <section className={section}><h2 className="font-serif text-xl font-bold">6. Supporting Evidence</h2><div className="grid gap-4 sm:grid-cols-2"><label className={label}>Supporting Document / Image<input className={field} type="file" disabled /></label><label className={label}>Additional Supporting Document<input className={field} type="file" disabled /></label></div><p className="text-xs text-[var(--ch-ink-muted)]">File-storage integration is not currently available.</p><label className={label}>Additional Reference / Notes<textarea className={field} name="supportingNotes" rows={3} placeholder="Add relevant notes, if applicable" maxLength={1000} /></label></section>
    <section className={section}><h2 className="font-serif text-xl font-bold">7. Follow-up Preference</h2><p className="text-sm text-[var(--ch-ink-muted)]">The preferred contact method selected above will be used for follow-up.</p><label className={label}>Preferred Contact Time<input className={field} name="preferredContactTime" placeholder="Enter preferred contact time" /></label><label className={label}>Additional Contact Instructions<textarea className={field} name="contactInstructions" rows={3} placeholder="Add contact instructions, if applicable" maxLength={1000} /></label></section>
    <section className={section}><h2 className="font-serif text-xl font-bold">8. Declaration &amp; Consent</h2><label className="flex gap-3 text-sm"><input name="accuracy" type="checkbox" required />I confirm that the information provided in this complaint is accurate to the best of my knowledge. *</label><label className="flex gap-3 text-sm"><input name="reviewConsent" type="checkbox" required />I consent to Holy Yatra reviewing the information and supporting material submitted for the purpose of investigating and responding to this complaint. *</label><label className="flex gap-3 text-sm"><input name="policyConsent" type="checkbox" required />I agree to the <Link className="font-semibold text-[var(--ch-accent)] underline" href="/privacy-policy">Privacy Policy</Link> and applicable <Link className="font-semibold text-[var(--ch-accent)] underline" href="/booking-terms">Booking Terms &amp; Conditions</Link>. *</label></section>
    {error ? <p className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800" role="alert">{error}</p> : null}
    <label className="flex gap-3 rounded-xl border border-[color:var(--ch-hairline)] p-4 text-sm"><input name="outcomeAcknowledgement" type="checkbox" required />I understand that if I submit incorrect information, the Company does not guarantee a particular resolution, refund, compensation, disciplinary action, or other requested outcome. *</label>
    <button className="rounded-full bg-[var(--ch-accent-soft)] px-6 py-3 font-semibold text-[var(--ch-ink)] disabled:opacity-50" disabled={submitting} type="submit">{submitting ? 'Submitting Complaint...' : 'Submit Complaint'}</button>
  </form>;
}
